...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    plt.plot(df['date'], df['order_count'], label='Orders per day', color='green')

    # Marcar los días festivos con líneas rojas
    holidays = df[df['holiday'] == True]['date']
    for h in holidays:
        plt.axvline(x=h, color='blue', linestyle='--', alpha=0.5)
